...
def input_command(command):
    ...
    if 'elizabeth' in  command:
        ...
        output('Si amo?... necesitas algo?')
    elif 'cuál es tu nombre' in command:
        output('mi nombre es Elizabeth, encantada de conocerte')
    elif 'whatsapp' in command or "abre what's up" in command:
        output('whatssap abierto', 'ara_ara_uhuhu.mp3')
        sleep(1)
        os.system('whatsapp-for-linux &')
        output("whatsapp abierto")

    elif "inicia el reconocimiento de objetos" in command:
        output("Iniciando reconocimiento de objetos...")
        os.system('python3 ~/holberton/capstone/object_detection/yolov7/detect.py --weights ~/holberton/capstone/object_detection/yolov7/yolov7.pt --conf 0.25 --img-size 640 --source 0 &')
    elif "cierra what's up" in command:
        print("cerrando...")
        output("whatsapp cerrado")
        os.system('pkill -9 whatsapp-for-li')
    elif 'abrir correo' in command:
        output('correo abierto', 'ara_ara_uhuhu.mp3')
        os.system('prospect-mail &')
        sleep(1)
        output("correo abierto")
    elif 'cerrar correo' in command:
        os.system('pkill -9 prospect-mail')
        output("correo cerrado")
    elif 'abrir discord' in command or 'open discourse' in command:
        output('email discord', 'ara_ara_uhuhu.mp3')
        os.system('discord &')
        sleep(1)
        output("discord abierto")
    ...
    elif 'desconéctate' in command or 'adiós elizabeth' in command:
        output('good bye', 'konichiwa_sayonara.mp3')
        exit()
    elif 'fuck you' in command:
        output('angry', 'angry.mp3')
    elif 'transferencia de estilo' in command:
        output('Tomando foto...')
        Streamming()
        output('Elige el estilo que desea aplicar')
        Show_Styles()
        command = get_audio_vosk()
        print(command)
        command = command.split()[1]
        print("SPLITTED " ,command)
        output('Aplicando estilo...')
        Pick_Style(command)
    elif "estoy bien también" in command:
...
def grettings():
    ...
    if hour >= 6 and hour < 12:
        output("Buenos dias amo, mi nombre es Elizabeth", 'OhayouGozaimasu.mp3')
...
# Loading vosk model
model = Model(r"/home/luffy/holberton/virtual_assistant/spanish_model")
...
while True:
    ...
    if bool(prediction[0]) is False:
        output("usuario no  válido")
        continue
    else:
        output("Bienvenido cariño")
        break
    
output("Iniciando reconocimiento facial...")

while True:
    result = face_analize()
    if result:
        output("Bienvenido amo...")
        break
    else:
        output("Reconocimiento fallido, porfavor intenta otra vez")
grettings()
